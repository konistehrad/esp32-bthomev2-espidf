...
void BTHome::begin(const char* dname, bool encryption, const char* key, bool trigger_based_device) {
...
void BTHome::setDeviceName(const char*  dname) {
  std::string str = std::string(dname);
  if (!str.empty())
    this->dev_name = std::move(str);
}
...
void BTHome::addMeasurement_state(uint8_t sensor_id, uint8_t state, uint8_t steps) {
  if ((this->m_sensorDataIdx + 2 + (steps > 0 ? 1 : 0)) <= (MEASUREMENT_MAX_LEN - (this->m_encryptEnable ? 8 : 0))) {
    this->m_sensorData[this->m_sensorDataIdx] = sensor_id & 0xff;
    this->m_sensorDataIdx++;
    this->m_sensorData[this->m_sensorDataIdx] = state & 0xff;
    this->m_sensorDataIdx++;
    if (steps > 0) {
      this->m_sensorData[this->m_sensorDataIdx] = steps & 0xff;
      this->m_sensorDataIdx++;
    }
